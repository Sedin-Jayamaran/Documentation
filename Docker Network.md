...
![Bridge](./Bridge%20Network%2011.png)

### Key points:

- Each container gets a private IP from the bridge network.
- Those IPs work only inside the same Docker host.
- To reach a container from outside, Docker does NAT (Network Address Translation) through the host’s public IP using port mapping (e.g., -p 8080:80).
- Without mapping, containers are isolated from public networks.
